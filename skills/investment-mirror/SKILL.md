...
- Keep interview calibration inside the structured evidence contract as one source row unless the profile is provisional and must list unknown dimensions. The rendered final HTML should stay compact: show evidence-boundary metrics and dimension-level evidence signals rather than a self-justifying evidence table by default.
